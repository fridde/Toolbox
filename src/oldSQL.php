<?php
	...
	class oldSQL{
		...
		{
			$this->connect();
			if(isset($this->configuration["default_table"])){
				$this->table = $this->configuration["default_table"];
			}
			echo "";
		}
		...
		*/
		...
		public function connect() {
			...
			*/
			...
			$this->getConnDetails();
			$cd = $this->configuration;
			
			$db_text = "mysql:host=" . $cd["db_host"] . ";dbname=". $cd["db_name"];
			$conn = new \PDO($db_text, $cd["db_username"], $cd["db_password"]);
		...
		public function getConnDetails()
		{
			if(is_readable($this->config_file)){
				...
				if(isset($configuration["Connection_Details"])){
					$this->configuration = $configuration["Connection_Details"];
				}
				else {
		...
		*/
		...
		function where($condition, $values = array())
		{
			if($this->where_clause == 1){
				$this->where_clause = $condition;
			}
			else {
				$this->where_clause .= " AND $condition";
			}
		...
		*/
		...
		function whereEqual($field, $value, $reverse = false)
		{
			...
			$operator = $reverse ? '<>' : '=';
			if($this->where_clause === 1){
				$this->where_clause = "$field $operator ?";
			}
			else {
				$this->where_clause .= " AND $field $operator ?";
			}
			$this->parameters[] = $value;
		...
		*/
		...
		function whereIn($field, $values, $reverse = false)
		{
			$field = $this->escapeField($field);
			...
			$in_array = array_fill(0, count($values), "?");
			$this->where_clause = implode(", ", $in_array) . ")";
			return $this;
		}
		...
		*/
		...
		function whereNull($field, $reverse = false)
		{
			...
			$this->where_clause .= " AND $field $operator NULL";
		...
		*/
		...
		function select()
		{
			$this->where_clause = (isset($this->where_clause) ? $this->where_clause : 1);
			$statement = "SELECT * FROM `$this->table` WHERE $this->where_clause";
			if(isset($this->order_clause)){
				$statement .= " ORDER BY $this->order_clause";
			}
			if(isset($this->limit_clause)){
				$statement .= " LIMIT $this->limit_clause";
			}
		...
		*/
		...
		function update($data)
		{
			...
			foreach ($fields as $field){
				$statement_array[] = "`$field` = ?";
			}
			$statement .= implode(", ", $statement_array);
			$statement .= " WHERE $this->where_clause";
		...
		*/
		...
		function delete()
		{
			$statement = "DELETE FROM $this->table WHERE $this->where_clause";
			$this->execute($statement, $this->parameters);
		}
		...
		function execute($statement)
		{
			...
			$PDOStatement->execute($this->parameters);
			return $PDOStatement;
		}
